...
def get_log():
    file_handler = logging.FileHandler(filename='import_export.log', mode='a', encoding='utf-8')
...
def connect_mongo(func):
    ...
    def inner(*args, **kwargs):
        ...
        res = ip_pattern.match(_ip)
        if res:
            err_one_lb['text'] = ''
            if _port is None:
                _port = 27017
            connect('measuresystem', host=_ip, port=int(_port))
            ret = func(*args, **kwargs)
            disconnect()
        else:
            err_one_lb['text'] = f'MongoDB ip is not conform to the rules!'
            ret = None
...
def save_qubit(yaml_file):
    logging.info(f'Qubit yaml file: {yaml_file}')
    ...
    qubit_store.save()
    logging.info(f'Save q{qubit_store.bit} info to QubitStore success, id: {qubit_store.id}\n')
...
def export_qubit(bit=None, qubit_id=None, export_path=None):
    ...
    logging.info(f'Export qubit info from QubitStore, id: {qubit_doc.id}')
    ...
    logging.info(f'Export q{qubit_doc.bit} info to yaml file success, file: {file}\n')
...
def export_click():
    ...
    def export_sure():
        ...
        if bit or qubit_id:
            ...
            if bit:
                bit = int(bit)
            logging.info(f'Export qubit bit: {bit}, id: {qubit_id}')
...
if __name__ == '__main__':
    ...
    lb = tkinter.Label(window, text='', width=16, height=1)
    lb.pack()

    ip_lb = tkinter.Label(window, text='MongoDB IP', width=16, height=2)
    ip_lb.pack()
    ip_addr = tkinter.StringVar(value='127.0.0.1')
    ip_en = tkinter.Entry(window, textvariable=ip_addr, font=('Arial', 10), width=32)
    ip_en.pack()

    port_lb = tkinter.Label(window, text='MongoDB Port', width=16, height=2)
    port_lb.pack()

    port = tkinter.StringVar(value='27017')
    port_en = tkinter.Entry(window, textvariable=port, font=('Arial', 10), width=32)
    port_en.pack()

    err_one_lb = tkinter.Label(window, text='', fg="#FF0000", width=36, height=1)
    err_one_lb.pack()
    lb = tkinter.Label(window, text='', width=16, height=1)
    lb.pack()
